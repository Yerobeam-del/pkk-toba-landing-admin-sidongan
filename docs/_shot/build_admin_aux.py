# -*- coding: utf-8 -*-
"""Build standalone A4 cover + merged print-ready HTML for the admin manual."""
import os, re

ROOT = os.path.dirname(os.path.dirname(os.path.abspath(__file__)))  # docs/
MANUAL = os.path.join(ROOT, 'user-manual-admin-panel.html')
COVER_OUT = os.path.join(ROOT, 'sampul-admin-panel.html')
MERGED_OUT = os.path.join(ROOT, '_shot', 'manual-print-merged-admin.html')
...
doc = ('<!DOCTYPE html>\n<html lang="id">\n<head>\n<meta charset="UTF-8">\n'
       ...
       '<title>User Manual Admin Panel — Sampul</title>\n<style>\n' + style + '\n' + overrides + '\n</style>\n'
       '</head>\n<body>\n' + cover + '\n</body>\n</html>\n')
