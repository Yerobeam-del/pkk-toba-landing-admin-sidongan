...
<aside class="sidebar" id="sidebar">
    ...
    <nav class="sidebar-nav">
        ...
        @if($currentUser && $currentUser->isSidonganKetua())
            ...
            {{-- Tautan Surat sudah ada di submenu blok Sekretaris — jangan tampilkan dua kali --}}
        ...
        {{-- Super Admin tidak ikut: kedua tautannya sudah ada di blok Sekretaris/Ketua --}}
        @if($currentUser && !$currentUser->isSuperAdmin() && ($currentUser->isSidonganBendahara() || $currentUser->isSidonganPokja() || $currentUser->isSidonganStafAhli()))
            <a href="{{ route('sidongan.documents.index') }}" class="nav-item {{ request()->routeIs('sidongan.documents.*') ? 'active' : '' }}">
                <div class="nav-icon-box">
                    <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2">
