...
<div style="display:flex;justify-content:space-between;align-items:center;gap:1rem;margin-bottom:1.5rem;flex-wrap:wrap">
    {{-- Search Form --}}
    <div class="template-search-wrapper" style="flex:1;min-width:200px">
        <form method="GET" action="{{ route('admin.template.index') }}">
            ...
            <div style="position:relative">
                ...
                <input type="text" name="search" value="{{ request('search') }}" class="template-search-input" placeholder="Cari template..." style="padding:0.5rem 0.75rem 0.5rem 2.5rem;border:1px solid rgba(0,0,0,0.06);border-radius:8px;font-size:0.9rem;width:100%;transition:all 0.2s" onfocus="this.style.borderColor='var(--primary)';this.style.boxShadow='0 0 0 3px rgba(13, 148, 136, 0.1)'" onblur="this.style.borderColor='rgba(0,0,0,0.06)';this.style.boxShadow='none'">
                @if(request('search'))
    ...
    {{-- Per Page Dropdown --}}
    <div class="template-perpage-wrapper" style="display:flex;align-items:center;gap:0.5rem;flex-shrink:0">
        <form method="GET" action="{{ route('admin.template.index') }}" class="template-form-wrapper" style="display:flex;align-items:center;gap:0.5rem">
            <input type="hidden" name="tab" value="{{ $currentTab }}">
